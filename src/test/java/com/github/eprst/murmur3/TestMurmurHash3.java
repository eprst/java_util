...
public class TestMurmurHash3 extends TestCase {
  ...
  private void doString(String s) {
    ...
    doString(s, 0, 0, false, seed);
  }

  private void doString(String s, int pre, int post, boolean ascii, int seed) {
    byte[] utf8 = s.getBytes(utf8Charset);
    int hash1 = MurmurHash3.murmurhash3_x86_32(utf8, pre, utf8.length - pre - post, seed);
    int hash2 = MurmurHash3.murmurhash3_x86_32(s, pre, s.length() - pre - post, seed);
    if (hash1 != hash2) {
...
//    HashFunction guavaHash32 = Hashing.murmur3_32(seed);
...
//    assertEquals(guava32.asInt(), hash1);
    ...
    MurmurHash3.murmurhash3_x64_128(utf8, pre, utf8.length - pre - post, seed, r1);
    MurmurHash3.murmurhash3_x64_128(s, pre, s.length() - pre - post, seed, null, r2);
    assertEquals(r1, r2);
    HashFunction guavaHash128 = Hashing.murmur3_128(seed);
    ...
    if (ascii) {
      MurmurHash3.murmurhash3_x64_128_ascii(s, pre, s.length() - pre - post, seed, r2);
  ...
  public void testStringHash() {
    ...
    for (int i = 0; i < 100000; i++) {
      ...
      doString(s);
      doString(middle);
      doString(s, pre, post, false, Math.abs(r.nextInt()));
    }
  ...
  public void testRandomBytes() {
    ...
    for (int i = 0; i < 1000; i++) {
      ...
      MurmurHash3.murmurhash3_x64_128(bytes, 0, bytes.length, seed, c);
      String murmur3Result = c.toString();

      String guavaResult = Hashing.murmur3_128(seed).hashBytes(bytes).toString();

      String msg = "seed: " + seed + ", bytes: " + Arrays.toString(bytes);
      assertEquals(msg, guavaResult, murmur3Result);
    }
  }
  ...
  public void testRandomAscii() {
    ...
    for (int i = 0; i < 10000; i++) {
      ...
      doString(s, 0, 0, true, Math.abs(r.nextInt()));
    }
  }
  ...
  public void testRandomUnicode() {
    ...
    for (int i = 0; i < 10000; i++) {
      int len = r.nextInt(256);
      doString(rsg.randomUnicode(len));
    }
    for (int i = 0; i < 100; i++) {
      int len = r.nextInt(1500);
      doString(rsg.randomUnicode(len));
    }
    for (int i = 0; i < 10; i++) {
      int len = r.nextInt(4000);
      doString(rsg.randomUnicode(len));
    }
  }
